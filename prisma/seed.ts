...
const allNodes = [...conceptEntities, ...theoremEntities, ...methodEntities, ...applicationEntities];
...
async function seed() {
  ...
  const existingNodes = await db.kGNode.count();
  if (existingNodes > 0) {
    console.log(`Database already has ${existingNodes} nodes, skipping seed.`);
    return;
  }
